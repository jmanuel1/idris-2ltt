...
data Sub : (0 t1 : var -> Ty var Val) -> Ty var Val -> Ty var Val -> Type where
  [search t1]
  SubFix : (0 t1 : var -> var -> Ty var Val) -> (forall a. Sub (\a' => t1 a' a) t (t1' a)) -> Sub (\a => Fix (t1 a)) t (Fix t1')
  ...
  -- `TreeExample.leaf`).
  SubSum : Sub t1 t t1' -> (0 t2s : var -> Vect n (Ty var Val)) -> {0 t2s' : Vect n _} -> Sub (\a => Sum (t2s a)) t (Sum t2s') -> Sub (\a => Sum (t1 a :: t2s a)) t (Sum (t1' :: t2s'))
  SubProd :
    Sub t1 t t1' ->
    (0 t2s : var -> Vect n (Ty var Val)) ->
    {0 t2s' : Vect n _} ->
